...
clc
clear 
grayDestinationImageName = 'desImage05.jpg';
colorSourceImageName= 'colorImage05.jpg';
subplot(1,3,1);imshow(colorSourceImageName);
...
destinationImage = imresize(destinationImage,1);

%destinationImage = rgb2gray(destinationImage);
colorSourceImage = imread(colorSourceImageName);
colorSourceImage = imresize(colorSourceImage,0.3);
...
subplot(1,3,2);imshow(lab2rgb(labDesImageMatrix));
...
for i = 1:souHeight
    for j = 1:souWidth
        PowerSouImageMatrix(i,j) = LSouImageMatrix(i,j) / 2 + neihoodMeanLSouImageMatrix(i,j) / 2;
    end
end
...
subplot(1,3,3);imshow(resultImageMatrix);
