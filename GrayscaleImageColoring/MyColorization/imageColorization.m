function [finalImageMatrix] = imageColorization(desImageMatrix,souImageMatrix,imageSize)
%% 1)Read Grey Destination Image And Colorful Source Image And Resize Images
disp('Step One : Read Images');
resizeScale = imageSize;
DesGreyImage = desImageMatrix;
...
[SegDesGreyLabelMatrix, SegDesGreyLabelNum] = mySLIC(DesGreyImage,300);
[SegSouColorLabelMatrix, SegSouColorLabelNum] =  mySLIC(SouColorImage,300);
...
colorizedImageMatrix = PixelNodeColorization(desPixelMatrix,souSegLabelMax,desMatchLabelMatrix,souPixelMatrix,labDesGreyImage);
...
figure;
subplot(2,2,1);imshow(SouColorImage);
subplot(2,2,2);imshow(DesGreyImage);
subplot(2,2,3);imshow(colorizedImageMatrix);
subplot(2,2,4);imshow(finalImageMatrix);
figure;
imshow(finalImageMatrix);

%%Colorization Done
disp('Colorization Done!');
end
%%
...
%%
function [outColorizedMatrix] = PixelNodeColorization(inDesPixelMatrix,inSouLabelMax,inMatchLabelMatrix,inSouPixelMatrix,inDesInitialMatrix)
...
for superpixel_i = 1 : inSouLabelMax
    matchSuperPixelVector = zeros(1,inSouLabelMax);
    for desHeight_i = 1 :desImageHeight
        for desWidth_j = 1 : desImageWidth
